...
import { Pencil, Trash2, Truck, Phone, Mail } from 'lucide-react'
...
export function SupplierList({ suppliers, loading, onEdit, onDelete }: SupplierListProps) {
  if (loading) {
    return (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-0.5">
              {s.contact && (
                <span className="text-xs text-muted-foreground truncate">{s.contact}</span>
              )}
              {s.phone && (
